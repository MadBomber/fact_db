...
module FactDb
  module Loaders
    ...
    # 1. $XDG_CONFIG_HOME/fact_db/fact_db.yml (if XDG_CONFIG_HOME is set)
    # 2. ~/.config/fact_db/fact_db.yml (XDG default fallback)
    #
    # On macOS, also checks:
    # 3. ~/Library/Application Support/fact_db/fact_db.yml
    #
    ...
    #   ~/.config/fact_db/fact_db.yml
    #
    # @example Custom XDG_CONFIG_HOME
    #   export XDG_CONFIG_HOME=/my/config
    #   # Looks for /my/config/fact_db/fact_db.yml
    #
    class XdgConfigLoader < Anyway::Loaders::Base
      class << self
        ...
        def config_paths
          ...
          if macos?
            macos_path = File.expand_path("~/Library/Application Support/fact_db")
            paths << macos_path if Dir.exist?(File.dirname(macos_path))
          end

          # XDG default: ~/.config/fact_db
          xdg_default = File.expand_path("~/.config/fact_db")
          paths << xdg_default

          # XDG_CONFIG_HOME override (highest priority for XDG loader)
          if ENV["XDG_CONFIG_HOME"] && !ENV["XDG_CONFIG_HOME"].empty?
            xdg_home = File.join(ENV["XDG_CONFIG_HOME"], "fact_db")
        ...
        #
        # @param name [String] config name (e.g., 'fact_db')
      ...
      def load_yaml(path, name)
        ...
        warn "FactDb: Failed to parse XDG config #{path}: #{e.message}"
        {}
      end
    end
  end
...
Anyway.loaders.insert_before :yml, :xdg, FactDb::Loaders::XdgConfigLoader
