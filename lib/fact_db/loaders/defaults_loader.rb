...
module FactDb
  module Loaders
    ...
    # This ensures defaults are always available regardless of where FactDb is installed.
    #
    ...
    class DefaultsLoader < Anyway::Loaders::Base
      ...
      class << self
        ...
        def load_raw_yaml
          ...
          warn "FactDb: Failed to parse bundled defaults #{defaults_path}: #{e.message}"
          {}
        end
      ...
      #
      # Priority: FDB_ENV > RAILS_ENV > RACK_ENV > 'development'
      #
      # @return [String] current environment name
      def current_environment
        ENV["FDB_ENV"] || ENV["RAILS_ENV"] || ENV["RACK_ENV"] || "development"
      end
    end
  end
end
...
Anyway.loaders.insert_before :yml, :bundled_defaults, FactDb::Loaders::DefaultsLoader
